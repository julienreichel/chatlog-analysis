...
The sandbox generates `amplify_outputs.json` at the repo root with the Cognito User Pool ID, client ID, and region. The app reads this file automatically – no manual environment variable entry is needed.
If you want custom table names in `amplify/backend.ts`, set `DYNAMO_TABLE_NAME` and `DYNAMO_ANALYSIS_TABLE_NAME` in the shell before running `npx ampx ...` (for example: `DYNAMO_TABLE_NAME=... npx ampx sandbox`).
...
> **Cognito settings** (`NUXT_PUBLIC_COGNITO_USER_POOL_ID`, `NUXT_PUBLIC_COGNITO_USER_POOL_CLIENT_ID`, `NUXT_PUBLIC_COGNITO_REGION`) are read automatically from `amplify_outputs.json` generated by `npx ampx sandbox` or `npx ampx pipeline-deploy`. You only need to set them manually if you are not using Amplify to provision your Cognito User Pool.
>
> **About `DYNAMO_TABLE_NAME` in `amplify/backend.ts`:** that variable is read by Amplify CLI at synth/deploy time, not from `app/.env`. You do **not** need a root `.env` by default. Set it in your shell/CI environment only if you want non-default table names.
...
If you want custom DynamoDB table names for infrastructure creation itself, set the same `DYNAMO_*` variables in the environment where you run `npx ampx pipeline-deploy` / `npx ampx sandbox`.

> Cognito settings (`user_pool_id`, `user_pool_client_id`, `aws_region`) are read automatically from the `amplify_outputs.json` file that `npx ampx pipeline-deploy` generates. No manual entry is required.
